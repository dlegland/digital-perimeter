classdef (InferiorClasses = {?matlab.graphics.axis.Axes}) StarFish2D
% One-line description here, please.
%
%   Class StarFish2D
%
%   Example
%   shape = StarFish2D([50 50 40 25 10]);
...
%% Constructor
methods
    function obj = StarFish2D(varargin)
        % Constructor for StarFish2D class.
        ...
        switch nargin
            ...
            case 1
                var1 = varargin{1};
                if size(var1, 2) ~= 5
                    error('Creating a StarFish2D requires an array with five columns, not %d', size(var1, 2));
                end
                obj.CenterX = var1(1);
                obj.CenterY = var1(2);
                obj.Rout    = var1(3);
...
methods
    ...
    function res = isInside(obj, pts)
        ...
        rhoTh = (rOut+rIn)/2 + (rOut-rIn)/2 * cos(5 * (th - theta0));
...
methods
    ...
    function [x, y] = pointCoords(obj, t)
        ...
        rho = (rOut+rIn)/2 + (rOut-rIn)/2 .* cos(5 * (t - theta0));
...
methods (Static)
    function shape = fromStruct(str)
        ...
        shape = StarFish2D([str.CenterX str.CenterY str.OuterRadius str.InnerRadius str.Orientation]);
    end
end
...
function [obj, varargin] = parseDrawable(varargin)
    ...
    ind = cellfun(@(x) isa(x, 'StarFish2D'), varargin);
    obj = varargin{ind};
    varargin(ind) = [];
end
